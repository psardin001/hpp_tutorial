"""Plan one_gear from the measured arm pose and execute all or step through it."""
...
def main():
    ...
    parser.add_argument(
        "--execute", action="store_true", help="exécution réelle étape par étape"
    )
    parser.add_argument(
    ...
    if q_start is not None and not np.isfinite(q_start).all():
        parser.error("--start doit contenir six angles finis")
    ...
    namespace = runpy.run_path(
        str(Path(__file__).with_name("one_gear.py")),
    ...
    if args.execute:
        prompt_execution(json.loads(args.plan.read_text()))
